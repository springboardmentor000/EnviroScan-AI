...
STATIONS = [
    {"location_id": 3409492, "location": "HB Colony, Vijayawada - APPCB",  "lat": 16.5206, "lon": 80.6404},
    {"location_id" : 3409392, "location" : "Kanuru, Vijayawada - APPCB", "lat": 16.4890, "lon": 80.6940}
]
CITY = "Vijayawada"


def fetch_openmeteo(lat, lon, start="2026-01-01", end="2026-03-10"):
    resp = requests.get(
...
def fetch_owm_current(lat, lon):
    ...
    return [{
        "timestamp":      datetime.fromtimestamp(d["dt"]).strftime("%Y-%m-%dT%H:%M:%S"),
...
for station in STATIONS:
    print(f"\n📍 Station: {station['location']} (ID: {station['location_id']})")
...
# ✅ Updated column order with date and time columns
df = df[[
...
filename = f"vijayawada_weather.csv"
